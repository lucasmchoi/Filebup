...
for volume in configl.keys():
    if configl[volume]['service'] == 'borg':
        ...
        if volume in cronc:
            ...
            helpercreatesh += "dockernum=$(echo '$dockerids' | wc -l)\n"
            helpercreatesh += "ownshortid=$HOSTNAME\n"
            helpercreatesh += "for id in $dockerids\n"
            helpercreatesh += "do\n"
            helpercreatesh += "[[ $id = $ownshortid* ]] || curl -s -XPOST --unix-socket /var/run/docker.sock -H 'Content-Type: application/json' http:/containers/$id/stop\n"
            helpercreatesh += "done\n"
            helpercreatesh += "while true\n"
            helpercreatesh += "do\n"
            helpercreatesh += "currentnum=$(curl --silent -XGET --unix-socket /var/run/docker.sock http://localhost/containers/json | jq -r '.[].Id' | wc -l)\n"
            helpercreatesh += "if [ $currentnum -ne $dockernum ]\n"
            helpercreatesh += "then\n"
            helpercreatesh += "break\n"
            helpercreatesh += "fi\n"
            helpercreatesh += "sleep 1\n"
            helpercreatesh += "done\n"
            helpercreatesh += "m=$(borg create -s --show-version --show-rc -c 300 -C auto,lzma,6 $REPOSITORY::$(date +'%Y%m%d-%H%M') /filebup/volumes/{} 2>&1)\n".format(localpath)
            helpercreatesh += "for id in $dockerids\n"
            helpercreatesh += "do\n"
            helpercreatesh += "[[ $id = $ownshortid* ]] || curl -s -XPOST --unix-socket /var/run/docker.sock -H 'Content-Type: application/json' http:/containers/$id/start\n"
            helpercreatesh += "done\n"
            helpercreatesh += "while true\n"
            helpercreatesh += "do\n"
            helpercreatesh += "currentnum=$(curl --silent -XGET --unix-socket /var/run/docker.sock http://localhost/containers/json | jq -r '.[].Id' | wc -l)\n"
            helpercreatesh += "if [ $currentnum -eq $dockernum ]\n"
            helpercreatesh += "then\n"
            helpercreatesh += "break\n"
            helpercreatesh += "fi\n"
            helpercreatesh += "sleep 1\n"
            helpercreatesh += "done\n"
            helpercreatesh += "unset dockerids dockernum ownshortid currentnum\n"
