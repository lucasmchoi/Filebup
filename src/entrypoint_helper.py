...
for volume in configl.keys():
    if configl[volume]['service'] == 'borg':
        ...
        if volume in cronc:
            ...
            helpercreatesh += "do\n"
            helpercreatesh += "[[ $id = $ownshortid* ]] || curl -s -XPOST --unix-socket /var/run/docker.sock -H 'Content-Type: application/json' http://localhost/containers/$id/stop\n"
            helpercreatesh += "done\n"
            helpercreatesh += "m=$(borg create -s --show-version --show-rc -c 300 -C auto,lzma,6 $REPOSITORY::$(date +'%Y%m%d-%H%M') /filebup/volumes/{} 2>&1)\n".format(localpath)
            helpercreatesh += "for id in $dockerids\n"
            helpercreatesh += "do\n"
            helpercreatesh += "[[ $id = $ownshortid* ]] || curl -s -XPOST --unix-socket /var/run/docker.sock -H 'Content-Type: application/json' http://localhost/containers/$id/start\n"
            helpercreatesh += "done\n"
            helpercreatesh += "unset dockerids ownshortid currentnum\n"
